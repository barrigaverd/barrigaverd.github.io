...
setInterval(() => {
    ...
    if (obstacle) {
        ...
        const [jump, crouch] = dino.compute([distance, speed, height]);

        if (jump === crouch) return;
        if (jump) controls.dispatchEvent('jump'); //se for verdadeira o dino pula
        if (crouch) controls.dispatchEvent('crouch');
    };
}, 100);
